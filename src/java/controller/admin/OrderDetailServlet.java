...
//        List<Order> ls= od.Date();
         ArrayList<OrderDateDetail> list = new ArrayList<>();
        
            list.addAll(od.OrderDateDetail(date));   
        request.setAttribute("currentdate", date);
        request.setAttribute("ord", list);
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < ls.size(); i++) {
            list.addAll(od.OrderDateDetail(od.Date().get(i).getDate()));  
